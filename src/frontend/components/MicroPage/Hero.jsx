...
const MicroHero = ({ data, onLoadComplete }) => {
  ...
  useEffect(() => {
    ...
    // Check if 'client' is present in data
    if (data.micro_hero_section.client) {
      if (data.micro_hero_section.bangaloreFrames) {
        ...
        if (isMobile) {
          frameCount = import.meta.glob('/public/assets/images/micro/hero/client/mobile-new/*.webp');
        } else {
          frameCount = import.meta.glob('/public/assets/images/micro/hero/client/desktop-new/*.webp');
        }
      }
    } else {
      // Default case when there is no client
      frameCount = 10;
    }
  
    // Set the total frames by counting the matched files
    setTotalFrames(Object.keys(frameCount).length);
  }, [data, isMobile]);  // Re-run when `data` or `isMobile` changes
